<?php
  ...
  if(isset($_POST['btntampil'])) {
    ...
    $tglakhir   =isset($_POST['txttglakhir']) ? $_POST['txttglakhir']: date('m-Y');
              ...
              <div class="row align-items-center">
                <div class="col-xl-12">
                  <div class="card">
                    ...
                    <h5 class="card-header">Periode Tanggal <?php echo "$tglawal";?> S/D <?php echo "$tglakhir";?></h5>
                    <div class="card-body">
                      ...
                      <form action="<?php $_SERVER{'PHP_SELF'}; ?>" Method="POST" target="_SELF"  >
                        ...
                        <div class="row">
                          <div class="mb-3 row">
                            ...
                            <div class="col-md-5">
                              <input class="form-control" type="date" name="txttglawal" value="<?php echo($awaltgl); ?>" />
                             </div>
                           </div>
                          <div class="mb-3 row">
                            <label for="html5-text-input" class="col-md-2 col-form-label">Tanggal Akhir</label>
                            <div class="col-md-5">
                              <input class="form-control" type="date" name="txttglakhir" value="<?php echo($akhirtgl); ?>" />
                            </div>
                          </div>
                          <div class="mb-3 row">
                            <div class="col-md-5">
                              <button type="submit" name="btntampil" class="btn btn-primary">Tampilkan</button>
                                <a href="cetak1.php?awal=<?=$tglawal;?> && akhir=<?=$tglakhir;?> && sales=<?=$nama_sales;?>"class="btn btn-info">CETAK</a>
                            </div>
                         </div>
                    ...
                    <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search for names.." style="margin-bottom: 20px;border: 3px solid #30262621;display: block;padding: 5px;border-radius: 10px;margin-left: 5px;width: 20%; margin-top: 10px">
                      <table class="table table-bordered">
                          ...
                          <tbody id="myTable">
                            ...
                            if(isset($_POST['txttglawal']) && isset($_POST['txttglakhir'])){
                                // tampilkan data yang sesuai dengan range tanggal yang dicari 
                                $data = mysqli_query($con, "SELECT * FROM tb_jurnal_penjualan WHERE tanggal_penjualan BETWEEN '".$_POST['txttglawal']."' and '".$_POST['txttglakhir']."'");
                            }
                            
                            else{
                                //jika tidak ada tanggal dari dan tanggal ke maka tampilkan seluruh data
                                $data = mysqli_query($con, "select * from tb_jurnal_penjualan");   
                            }
                            ...
                            while($d = mysqli_fetch_array($data)){
                               ...
                               ?>
                            <tr>
                            <td><?php echo $d['id_jurnal_penjualan']; ?></td>
                            ...
                            <td>Rp. <?php echo number_format($d['total_penjualan'],0, ",", ".");?></td>
                            <td>
                                                    <a href="edit_jurnal_penjualan.php?id_jurnal_penjualan=<?= $d['id_jurnal_penjualan']; ?>" class="btn btn-info btn-sm">Edit</a>

                                                    <form action="code_penjualan.php" method="POST" class="d-inline">
                                                        <button type="submit" name="delete_jurnal_penjualan" value="<?=$d['id_jurnal_penjualan'];?>" class="btn btn-danger btn-sm">Delete</button>
                                                    </form>
                                                </td>
                            </tr>
                              <?php } ?>  
